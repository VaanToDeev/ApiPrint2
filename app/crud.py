...
async def create_estudante(db: AsyncSession, estudante: schemas.EstudanteCreate) -> models.Estudante:
    ...
    db_estudante = models.Estudante(
        ...
        curso_id=estudante.curso_id # Assuming curso_id is provided or handled
    )
    db.add(db_estudante)
    await db.commit()
...
async def create_professor(db: AsyncSession, professor: schemas.ProfessorCreate, role: models.UserRole = models.UserRole.PROFESSOR) -> models.Professor:
    ...
    db_professor = models.Professor(
        ...
        role=role # Assign role, could be admin during initial setup
    )
    db.add(db_professor)
    await db.commit()
...
async def assign_coordenador_to_curso(db: AsyncSession, curso_id: int, professor_id: int) -> Optional[models.Curso]:
    ...
    # Ensure professor has coordinator or admin role if that's a business rule
    # For now, any professor can be assigned.
    # Optionally, also update professor's role to COORDENADOR if they are not already ADMIN
    # if db_professor.role == models.UserRole.PROFESSOR:
    #    db_professor.role = models.UserRole.COORDENADOR

    db_curso.coordenador_id = professor_id
    # db_curso.coordenador = db_professor # This also works if relationship is set up correctly
    
    await db.commit()
    await db.refresh(db_curso)
    # if db_professor.role == models.UserRole.COORDENADOR: # Refresh if role was changed
    #    await db.refresh(db_professor)
    return db_curso